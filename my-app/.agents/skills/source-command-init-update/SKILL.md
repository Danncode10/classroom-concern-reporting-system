---
name: "source-command-init-update"
...
# source-command-init-update

Use this skill when the user asks to run the migrated source command `init-update`.
...
- `.Codex/` (commands, AGENTS.md, SKILLS.md, skill packs)
...
0. **Guard ruflo customizations** — before pulling anything, search ruflo memory for project-specific customizations baked into commands, AGENTS.md, or SKILLS.md (e.g. "ruflo memory protocol section added to AGENTS.md", "new-feature upgraded with parallel agents"). List them. After the update completes, verify these customizations survived — if any were overwritten, re-apply them automatically or prompt the user to run `/ruflo-upgrade`.
...
3. **For each selected category, show a diff:**
   ...
   **A) .Codex/ updates:**
   - Diff `.Codex/commands/` → show new, changed, deleted commands
   - Diff `.Codex/AGENTS.md` → show changes to project config
   - Show which skill packs are new/updated
   - Ask: "Merge .Codex/ updates?" (yes/no)
...
4. **Merge all approved updates:**
   ...
   - For `.Codex/AGENTS.md`, `.env.example`, `package.json`: use smart merge (show conflicts, ask user to resolve)
...
5. **Generate a single commit** with a clear summary:
   ```bash
   git add .Codex/ install.sh guide.sh package.json README.md docs/ 2>/dev/null || true
   git commit -m "chore(dannflow): update to latest version
   
   Updated:
   - .Codex/: 3 new commands, 2 updated, 5 new skill packs
...
6. **Report what changed:**
   ...
   .Codex/ updates:
     ...
     New skill packs: Codex-api, shadcn, a11y-audit
