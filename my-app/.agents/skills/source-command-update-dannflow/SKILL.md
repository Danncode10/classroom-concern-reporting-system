---
name: "source-command-update-dannflow"
...
# source-command-update-dannflow

Use this skill when the user asks to run the migrated source command `update-dannflow`.
...
Affected areas: .Codex/commands/, SKILLS.md
```

Generate the affected areas summary:
```bash
git diff --name-only <dannflow_commit> upstream/main -- \
  .Codex/commands/ .Codex/agents/ .github/ SKILLS.md AGENTS.md AGENTS.md \
